...
func New(option Option, log log.Logger) (messaging.Queue, error) {
...
func (k *kafka) ReadWithContext(ctx context.Context, topic string, callbacks []messaging.CallbackFunc) error {
	...
	for {
		...
		if err != nil {
			k.log.Error(context.Background(), "Kafka.ReadWithContext() ERROR read messages", nil, err)
			continue
		}

		for _, c := range callbacks {
			if err = c(m.Value); err != nil {
				k.log.Error(context.Background(), "Kafka.ReadWithContext() ERROR callbacks", nil, err)
			}
		}
	}
}
...
func (k *kafka) PublishWithContext(ctx context.Context, topic, message string) error {
	...
	if err := w.WriteMessages(context.Background(), kfk.Message{Value: []byte(message)}); err != nil {
		k.log.Error(context.Background(), "Kafka.PublishWithContext() ERROR write messages", nil, err)
...
func (k *kafka) Close() error {
	...
	for _, w := range k.writers {
		if e := w.Close(); e != nil {
			err = e
			k.log.Error(context.Background(), "Kafka.Close() ERROR writers", nil, err)
		}
	}
	...
	for _, r := range k.readers {
		if e := r.Close(); e != nil {
			err = e
			k.log.Error(context.Background(), "Kafka.Close() ERROR readers", nil, err)
		}
	}
